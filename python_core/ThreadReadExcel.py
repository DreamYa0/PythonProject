...
from python_core.MyThread import MyThread
...
class ExcelProcess(object):
    ...
    def get_case_list(self, filePath, case_line_list=[], startLine=0, endLine=0):
        ...
        if len(case_line_list) == 0 and startLine > 0 and endLine > 0:
            ...
            threads = []
            for i in range(10):
                t = MyThread(self.__get_by_xlrd, (filePath, case_line_list))
                threads.append(t)
            for i in range(10):
                threads[i].start()
            for i in range(10):
                threads[i].join()
                case_list = threads[i].getResult()
            return case_list
        else:
            raise ValueError(u'ExcelProcess: get_case_list: 传入参数错误'.encode('utf8'))

    def __get_by_xlrd(self, filePath, case_line_list, by_index=0):
        try:
            data = xlrd.open_workbook(filePath)
        except Exception:
            raise IOError(u"ExcelProcess: __open_excel: 解析文件异常请传入正确的Excel文件".encode('utf8'))
        try:
            table = data.sheets()[by_index]
        except Exception:
            raise IOError(u"ExcelProcess: __get_by_xlrd: data.sheets()[by_index]: by_index长度超过excel中的长度".encode('utf8'))
        nrows = table.nrows
        ...
        if len(RepeatDataList) > 0:
            raise Exception(u'ExcelProcess: __get_by_xlrd: 测试编号列中有重复元素,且重复元素为：%s' % json.dumps(RepeatDataList))
        ...
        if json.dumps(table.row_values(0), encoding='UTF-8', ensure_ascii=False) == json.dumps(
                ["用例编号", "名称", "描述", "系统", "服务", "接口", "数据初始化", "数据恢复", "参数", "预期结果"], encoding='UTF-8',
                ensure_ascii=False):
    ...
    def __get_by_value(self, filePath, startLine, endLine, by_index=0):
        ...
        if len(result) > 0:
            raise Exception(u'ExcelProcess: __get_by_xlrd: 测试编号列中有重复元素,且重复元素为：%s' % json.dumps(result))
        ...
        if json.dumps(table.row_values(0), encoding='UTF-8', ensure_ascii=False) == json.dumps(
            ...
            if 1 < startLine < endLine < nrows:
                ...
                raise ValueError(u'ExcelProcess: __get_by_value: 传入数据的起始行大于Excel文件最大行数或起始行小于1!'.encode('utf8'))
            return case_list
        else:
            raise IOError(u'ExcelProcess: __get_by_value: Excel表中的列名不正确或者列名顺序错误!')
...
if __name__ == "__main__":
    ...
    try:
        tables = excelProcess.get_case_list('E:\PythonProject\python_core\\files\Testcases_sku.xlsx',
                                            [])
        for row in tables:
